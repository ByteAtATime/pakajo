...
use super::legacy::Approvals;
use super::model::{Answer, ProviderCandidate, Question, QuestionKey};
...
pub struct LegacyApprovalsSource {
    approvals: Option<Approvals>,
}

impl LegacyApprovalsSource {
    pub fn new(approvals: Option<Approvals>) -> Self {
        Self { approvals }
    }
}

fn legacy_conflict_approved(approvals: &Approvals, incoming: &str, removable: &str) -> bool {
    approvals.approved_conflicts.iter().any(|conflict| {
        (conflict.incoming == incoming && conflict.removable == removable)
            || (conflict.incoming == removable && conflict.removable == incoming)
    })
}

fn legacy_provider_answer(
    approvals: &Approvals,
    depend: &str,
    ...
    let approval = approvals
        .approved_providers
        .iter()
        .find(|approval| approval.depend == depend)?;
    let found = match &approval.provider_repo {
        Some(required) => candidates.iter().any(|candidate| {
            candidate.name == approval.provider_name
                && candidate.repo.as_deref() == Some(required.as_str())
        }),
        None => candidates
            .iter()
            .any(|candidate| candidate.name == approval.provider_name),
    };
    found.then(|| Answer::SelectProvider {
        name: approval.provider_name.clone(),
        repo: approval.provider_repo.clone(),
    })
}

impl AnswerSource for LegacyApprovalsSource {
    fn answer(&self, question: &Question) -> SourceDecision {
        match question {
            Question::Replace { old, new, .. } => SourceDecision::Answer(Answer::Replace {
                old: old.clone(),
            ...
            Question::ImportKey { fingerprint, .. } => SourceDecision::Answer(Answer::ImportKey {
            ...
            Question::HoldPkgs { .. } => SourceDecision::Abort(FailClosed {
                key: question.key(),
                reason: "held package(s) require explicit override".to_string(),
            }),
            Question::Conflict {
                incoming,
                removable,
                ...
                Some(_) => SourceDecision::Abort(FailClosed {
                    key: question.key(),
                    reason: format!("declined to remove {removable}"),
                }),
                None => SourceDecision::Abort(FailClosed {
                    key: question.key(),
                    reason: format!(
            ...
            Question::SelectProvider { depend, candidates } => match &self.approvals {
                Some(approvals) => match legacy_provider_answer(approvals, depend, candidates) {
                    Some(answer) => SourceDecision::Answer(answer),
                    None => SourceDecision::Abort(FailClosed {
                        key: question.key(),
                        reason: format!("declined to choose a provider for {depend}"),
                    }),
                },
                None => SourceDecision::Abort(FailClosed {
                    key: question.key(),
...
mod tests {
    ...
    fn legacy_approvals() -> super::super::Approvals {
        super::super::Approvals {
            approved_conflicts: vec![super::super::Conflict {
                incoming: s("cava-git"),
                removable: s("cava"),
            }],
            approved_providers: vec![super::super::ProviderApproval {
                depend: s("sdl"),
                provider_name: s("sdl12-compat"),
    ...
    fn legacy_with(approvals: Option<super::super::Approvals>) -> LegacyApprovalsSource {
        LegacyApprovalsSource::new(approvals)
    }

    #[test]
    fn legacy_source_proceed_passes_through() {
        for approvals in [Some(legacy_approvals()), None] {
            let source = legacy_with(approvals);
            assert_eq!(
    ...
    #[test]
    fn legacy_source_approves_conflict_in_either_direction() {
        let source = legacy_with(Some(legacy_approvals()));
        for (incoming, removable) in [("cava-git", "cava"), ("cava", "cava-git")] {
            assert_eq!(
    ...
    #[test]
    fn legacy_source_denied_conflict_aborts_with_exact_string() {
        let source = legacy_with(Some(legacy_approvals()));
        let question = Question::Conflict {
            incoming: s("foo"),
            removable: s("bar"),
        ...
        assert_eq!(
            ...
            SourceDecision::Abort(FailClosed {
                key: question.key(),
                reason: "declined to remove bar".to_string(),
            })
        );
    }

    #[test]
    fn legacy_source_matches_provider_by_name_and_repo_pair() {
        let source = legacy_with(Some(legacy_approvals()));
        let question = Question::SelectProvider {
            depend: s("sdl"),
            candidates: vec![provider("sdl2", "extra"), provider("sdl12-compat", "extra")],
        };
        assert_eq!(
            source.answer(&question),
            SourceDecision::Answer(Answer::SelectProvider {
    ...
    #[test]
    fn legacy_source_denied_provider_aborts() {
        let source = legacy_with(Some(legacy_approvals()));
        let question = Question::SelectProvider {
            depend: s("libgl"),
            candidates: vec![provider("libglvnd", "extra")],
        };
        let SourceDecision::Abort(denied) = source.answer(&question) else {
            panic!("expected fail-closed for {question:?}");
        };
