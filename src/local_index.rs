...
mod tests {
    ...
    fn prefix_upper_bound_increments_last_char() {
        ...
        assert_eq!(
            prefix_upper_bound(&format!("a\u{10FFFF}")),
            Some("b".to_string())
        );
    }
    ...
    fn names_with_prefix_range_uses_index() {
        ...
        let plan: String = conn
            .prepare("EXPLAIN QUERY PLAN SELECT name FROM packages WHERE name >= ?1 AND name < ?2 ORDER BY name LIMIT ?3")
            .unwrap()
            .query_map(rusqlite::params!["a", "b", 5], |row| row.get::<_, String>(3))
            .unwrap()
        ...
        assert!(
            ...
            "query plan should use the primary key index: {plan}"
        );
    }
